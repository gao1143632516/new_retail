    ...
    <div class="">
        ...
        <div v-if="type=='upload'">
            <div class="handle-box el-upload-item">
                <i v-if="selectedImg==null||selectedImg==''" class="el-icon-plus" @click="showAttachment"></i>
            ...
            <el-dialog
                ...
                <div class="el-flex">
                    ...
                    <template v-for="(item,index) in attachmentList">
                        <div class="el-flex-item" :class="item.checked?'on':''" :key="index">
                            <el-checkbox v-model="item.checked"></el-checkbox>
                            <img @click="selectImg(index,item.attachment_id)" :src="item.url">
                            <i class="el-icon-close" @click="delAttachment(index,item)"></i>
                        </div>
                    </template>
                </div>
...
<script>
    ...
    import {getAttachmentList,delAttachment} from '@/api/file';
    export default {
        props: [
            "type",
            "selectedImg",
        ...
        data() {
            return {
                dialogVisible:false,
                uploadUrl:process.env.BASE_API+"/attachmentFile/uploadPath",
                attachmentList:[],
        ...
        methods:{
            ...
            getAttachmentList(pageNum,pageSize){
                getAttachmentList(this.dataPage).then((res)=>{
                    this.attachmentList = res.data.list;
                    this.dataPage.totalRow = res.data.num;
                })
                // if(pageNum==null)
            ...
            delAttachment(index,item){
                 delAttachment(item.attachment_id).then((res)=>{
                     if(res.code==0){
            ...
            handleUploadSuccess(res, file){
                ...
                // var attachment;
                // attachment.thumbnail_temp=res.data.url;
        ...
        created(){
            this.uploadUrl = this.uploadUrl+"?sessionkey="+this.sessionkey;
